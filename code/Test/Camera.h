...
public:
	...
	void updateKey(GLfloat delta_time, GLfloat speed)
	{
		...
		if ( Window::Get().GetKey(GLFW_KEY_W) )
		{
			camera_pos += velocity * camera_front;
		}
		if ( Window::Get().GetKey(GLFW_KEY_S) )
		{
			camera_pos -= velocity / 2 * camera_front;
		}
		if ( Window::Get().GetKey(GLFW_KEY_A) )
		{
			camera_pos -= camera_right * velocity;
		}
		if ( Window::Get().GetKey(GLFW_KEY_D) )
		{
			camera_pos += camera_right * velocity;
		}
		if ( Window::Get().GetKey(GLFW_KEY_SPACE) )
		{
			camera_pos += glm::vec3(0.0f, 0.01f * delta_time, 0.0f); // for jump
		}
		// zoom
		if ( Window::Get().GetKey(GLFW_KEY_LEFT_SHIFT) )
		{
			fov = 10.0f;
			sensitivity = 0.025f;
		}
