...
void Model::processNode(aiNode* node, const aiScene* scene)
{
	...
	for ( uint32_t i = 0; i < scene->mNumMeshes; i++ )
	{
		...
		meshes.push_back(mesh); //accumulate all meshes in one std::vector
	}
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	indices.reserve(mesh->mNumVertices); // дальше нада будет юзать std::vector.push_back(i);
										 ...
										 // поетому его елементы нада инициализировать сразу ( или потом дл€ каждого вызывать std::vector.push_back(i); перед использованием елемента )
	...
	for ( uint32_t i = 0; i < mesh->mNumBones; i++ )
	{
		...
		{
			uint32_t vertex_id = mesh->mBones[i]->mWeights[j].mVertexId; // ид вершины на которую вли€ет выбранна€ кость
...
uint32_t Model::findPosition(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for ( uint32_t i = 0; i < p_node_anim->mNumPositionKeys - 1; i++ ) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
uint32_t Model::findRotation(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for ( uint32_t i = 0; i < p_node_anim->mNumRotationKeys - 1; i++ ) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
uint32_t Model::findScaling(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for ( uint32_t i = 0; i < p_node_anim->mNumScalingKeys - 1; i++ ) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
aiVector3D Model::calcInterpolatedPosition(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	uint32_t position_index = findPosition(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	uint32_t next_position_index = position_index + 1; // индекс следующего ключевого кадра
...
aiQuaternion Model::calcInterpolatedRotation(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	uint32_t rotation_index = findRotation(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	uint32_t next_rotation_index = rotation_index + 1; // индекс следующего ключевого кадра
...
aiVector3D Model::calcInterpolatedScaling(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	uint32_t scaling_index = findScaling(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	uint32_t next_scaling_index = scaling_index + 1; // индекс следующего ключевого кадра
