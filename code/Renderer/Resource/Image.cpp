...
bool Image::Init(const std::string &filename)
{
	...
	// Load DDS
	if ( magic == DDS_MAGIC/* ||
		((data.data[0] == 'D' || data.data[0] == 'd') &&
		 (data.data[1] == 'D' || data.data[1] == 'd') &&
		 (data.data[2] == 'S' || data.data[2] == 's') 
		)*/)
	{
		data.Seek(0);
...
//--------------------------------------------------------------------
void Image::imageAlloc(TextureFormat::Enum format, uint16_t width, uint16_t height, uint16_t depth, uint16_t numLayers, bool cubeMap, bool hasMips, const uint8_t *data)
{
	const ImageBlockInfo &blockInfo = getBlockInfo(format);
	const uint16_t blockWidth = blockInfo.blockWidth;
	const uint16_t blockHeight = blockInfo.blockHeight;
...
//--------------------------------------------------------------------
bool Image::imageGetRawData(uint16_t _side, uint8_t _lod, uint32_t _size, ImageMip& _mip)
{
	...
	if ( m_ktx )
	{
		...
		for ( uint8_t lod = 0, num = m_numMips; lod < num; ++lod )
		{
			...
			Assert(size == imageSize && "KTX: Image size mismatch");
